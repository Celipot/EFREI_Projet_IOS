            ...
            if let data = data, error == nil {
                if let responseHttp = response as? HTTPURLResponse {
                    if responseHttp.statusCode==200 {
                        if let response = try?
                            JSONDecoder().decode(Records.self, from: data) {                            callback((nil, nil), response.records)
                            
                        }
                        else {                            callback((CustomError.parsingError, "parsing error"), nil)         }
    ...
    func deleteFurniture(with id: String, callback: @escaping ((errorType:CustomError?, errorMessage: String?), Response?) -> Void) {
        ...
        let task = session.dataTask(with: createRequest(urlStr: furnitureUrlStr,                                                        requestType: .delete,                                                        params: [id])) { (data, response, error) in
            if
                ...
                if let responseHttp = response as? HTTPURLResponse {
                    if responseHttp.statusCode==200 {
                        if let response = try?JSONDecoder().decode(Response.self, from: data) {                            callback((nil, nil), response)                        }
                        else if let response = try?
                                    JSONDecoder().decode(ErrorResponse.self, from:                         data) {
                            callback((CustomError.requestError,response.error), nil)                        }
                        else {
